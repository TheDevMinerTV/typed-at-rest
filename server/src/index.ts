...
export const handleCoreRequest = async <ED extends EndpointDefinition<any>>(
	...
	Effect.gen(function* (_) {
		...
		const params: Params = yield* _(
			query,
			S.validate(endpoint.path.schema),
			Effect.mapError((errors) =>
				makeAPIErrResult(400, `Could not parse query params with schema: ${formatError(errors)}`)
			)
		);
		...
		if ("request" in r) {
			validated = yield* _(
				body,
				S.validate(r.request),
				Effect.mapError((errors) =>
					makeAPIErrResult(400, `Could not parse json with schema: ${formatError(errors)}`)
				)
			);
		}
		...
		const res = yield* _(
			...
			Effect.mapError((e) => makeAPIErrResult(500, `Could not encode response with schema: ${formatError(e)}`))
		);
